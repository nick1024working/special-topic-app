...
@Component({
    selector: 'app-ub-manage-sale-tag',
    standalone: true,
    imports: [CommonModule, FormsModule],
    templateUrl: './manage-sale-tag.component.html',
    styleUrls: [
        './manage-sale-tag.component.css',
        '../../styles/bs-custom-override.scss',
    ],
})
export class ManageSaleTagComponent {
    private readonly tagSvc = inject(SaleTagService);
    ...
    async load() {
        ...
        this.error.set(null);
        try {
            const data = await firstValueFrom(this.tagSvc.getAllSaleTags());
    ...
    async create(req: CreateSaleTagRequestDto) {
        ...
        try {
            const realId = await firstValueFrom(this.tagSvc.createSaleTag(req));
            this.toastSvc.info("新增標籤成功!");
    ...
    async update(id: number, req: UpdatePartialBookSaleTagRequestDto) {
        ...
        try {
            await firstValueFrom(this.tagSvc.updateSaleTag(id, req));
        } catch (e) {
            this.saleTagList.set(prev);
            this.error.set("更新失敗");
        }
    ...
    async updateOrder() {
        ...
        await firstValueFrom(this.tagSvc.updateAllSaleTagsOrder(req));
    }
    ...
    async remove(id: number) {
        ...
        try {
            await firstValueFrom(this.tagSvc.deleteSaleTag(id));
        } catch (e) {
            this.saleTagList.set(prev);
            console.error(e);
            this.error.set("刪除失敗");
