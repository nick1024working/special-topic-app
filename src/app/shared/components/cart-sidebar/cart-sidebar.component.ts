...
export class CartSidebarComponent {
    ...
    ngOnInit(): void {
        this.cart.set(this.getMockCart());
        this.recalculate();
    }
    ...
    private recalculate() {
        ...
        const nextCart = this.buildNextCart(cart);
        nextCart.subtotal = nextCart.items.reduce((sum, item) => sum + (item.unitPrice * item.quantity), 0);
        nextCart.grandTotal = nextCart.subtotal - nextCart.discountTotal - nextCart.shippingFee;
